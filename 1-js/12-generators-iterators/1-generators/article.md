...
Generatory mogą zwrócić wiele wartości, być może ich nieskończoną ilość, jedna po drugiej, na żądanie
...
Generator można rozumieć jako "zawieszone wywołanie funkcji":
Po stworzeniu generatora, wykonanie kodu jest zapauzowane na samym początku.

Główną metodą generatora jest next(). Wywołana, wznawia wykonanie aż do następnego polecenia typu "yield". Wtedy wykonanie zostaje zatrzymane, a wartość zostaje zwrócona na zewnątrz.
...
Jeśli wywołamy ją po raz trzeci, wykonanie zostanie wznowione aż do następnego polecenia typu "return", które to kończy funkcje:
...
Mimo to, w większości przypadków używamy pierwszej składni, ponieważ gwiazdka * wskazuje, że to funkcja generująca, tzn opisuje rodzaj funkcji. 

## Na generatorach można dokonywać iteracji.

Jak wskazuje metoda `next()`, po generatorach moożna iterować (info: iterable)
...
...Weź prosze pod uwagę: przykład wyżej drukuje 1, następnie 2 i to wszystko. Nie drukuje 3!

Jest tak dlatego, ponieważ pętla for-of ignoruje ostatnią wartość, jeżeli done: true. Jeśli chcemy drukować wszystkie rezultaty za pomocą pętli for..of, to musimy je zwrócić za pomocą yield:
...
Oczywiście, po generatorach można iterować, więc można też wywoływać na nich funkcjonalności powiązane z iteracją, np operator rozprzestrzenienia ...:
...
W kodzie powyżej, ...generateSequence() zmienia iterable w tablice wartości (przeczytaj więcej o operatorze rozprzestrzenienia w rozdziale)

Korzystanie z generatorów zamiast z iterable.

Jakiś czas temu w rozdziale stworzyliśmy objekt typu iterable który zwraca wartości od..do.
...
Możemy wziąć najlepsze z obu dróg, przez dostarczenie generatora jako Symbol.iterator:
...
Na przykład, chcielibyśmy wygenerować ciąg:
    ...
    następujące po nich litery alfabetu a..z (kody znaków 65..90)
    następujące po nich wielkie litery A..Z (kody znaków 97..122)

Następnie, mamy zamiar wytwarzać hasła przez wybieranie znaków z tego ciągu, ale najpierw musimy wygenerować cią.
...
W przypadku zwykłej funkcji, aby połączyć ze sobą rezultaty kilku różnych funkcji, wywołujemy je, przechowujemy rezultaty, a na końcu łączym je ze sobą.
...
Aby to zrobić, powinniśmy wywołać generator.next(arg), z argumentem. Ten argument jest rezultatem "yield'a".
...
generator.next(4); // --> przekazuje rezultat do generatora 
	...
	Pierwsze wywołanie generator.next() jest zawsze bezargumentowe. Rozpoczyna ono wykonanie i zwraca rezultat pierwszego "yield'a" ("2+2?"). W tym momencie generator zatrzymuje wykonanie (nadal na tej samej linii).
    Następnie, jak pokazane na powyższym obrazku, rezultat "yield" zostaje przypisany zmiennej question w wywoływanym kodzie.
	Poprzez generator.next(4), generator wznawia wykonianie, i 4 zostaje przypisane zmiennej result: let result = 4.
...
Przebieg wykonania:
	Pierwsze .next() rozpoczyna wykonanie... Dochodzi do pierwszego "yield'a".
	Rezultat zostaje przekazany do kodu zewnętrznego.
	Drugi .next(4) przekazuje 4 z powrotem do generatora jako rezultat pierwszego "yield'a", a następnie kontynuuje wykonanie.
	...Wykonanie dochodzi do drugiego "yield'a", ten staje sie rezultatem wywołania generatora.
	Trzeci next(9) przekazuje 9 do generatora, jako rezultat drugiego "yield'a", następnie kontynuuje wykonywanie, które dochodzi do końca funkcji, a więc done: true.

To przypomina gre w "ping-pong". Każdy next(value) (za wyjątkiem pierwszego) przekazuje wartość do generatora, która staje sie rezultatem aktualnego "yield'a", a następnie przekazuje rezultat następnego "yield'a".

generator.throw

Jak zauważyliśmy w przykładach wyżej, kod zewnętrzny może przekazać wartość do generatora, jako rezultat "yield'a".
As we observed in the examples above, the outer code may pass a value into the generator, as the result of yield.

...Ale może też zainicjować (rzucić) błąd. Jest to naturalne, ponieważ błąd też jest rodzajem rezultatu. 

Aby przekazać błąd do "yield'a", powinniśmy wywołać generator.throw(err). W tym przypadku, err jest rzucony w linijce razem z danym "yield'em".
...
Jeśli nie złapiemy go, to tak samo, jak w przypadku każdego innego wyjątku, "wywala" generator do kodu wywołującego.
